...
def log_bug(file: str, description: str, fix: str = ""):
    ...
    c.execute(
        ...
        (
            ...
            description,
            fix,
            datetime.now().isoformat() if fix else None,
        ),
    )
    conn.commit()
    conn.close()
...
def get_recent_context(agent: str = None, limit: int = 5) -> str:
    ...
    if agent:
        c.execute(
            "SELECT timestamp, task, action, result, status FROM episodes WHERE agent = ? ORDER BY id DESC LIMIT ?",
            (agent, limit),
        )
    else:
        c.execute(
            "SELECT timestamp, task, action, result, status FROM episodes ORDER BY id DESC LIMIT ?",
            (limit,),
        )
