...
def _call_llm(messages: list[dict], agent: str = "executive") -> dict:
    ...
    if HAS_LLM_CLIENT:
        try:
            ...
            print(
                f"[DEBUG] Parsed: tools={len(parsed.get('tools', []))} response={parsed.get('response', '')[:80]!r}"
            )
            return parsed
    ...
    def _build_messages(self, user_input: str, iteration: int = 1) -> list[dict]:
        ...
        for msg in history:
            ...
            if msg["role"] in ("user", "assistant"):
                ...
                messages.append(
                    {"role": "user", "content": f"[РЕЗУЛЬТАТ] {msg['content'][:4000]}"}
                )
        if iteration == 1:
...
def handle_command(cmd: str, agent: AtlasCodeAgent) -> str | None:
    ...
    if command == "/help":
        ...
        result = subprocess.run(
            ["python", "-m", "py_compile", "atlas_core/agent.py"],
            capture_output=True,
            text=True,
        )
        ...
        print(
            "\n".join(
                [
                    f"• {sug['title']}: {sug['description'][:100]}..."
                    for sug in s.suggestions[-5:]
                ]
            )
        )
        ...
        planner_config = yaml.safe_load(
            open("agents/planner/agent.yaml", encoding="utf-8")
        )
        ...
        reviewer_config = yaml.safe_load(
            open("agents/reviewer/agent.yaml", encoding="utf-8")
        )
        ...
        if action == "rename_class":
            if len(args) < 4:
                print(
                    "❌ Использование: /refactor rename_class <file> <old_name> <new_name>"
                )
                return
            ...
            if len(args) < 4:
                print(
                    "❌ Использование: /refactor move_function <src> <dst> <func_name>"
                )
                return
        ...
        f = Formatter()
        if arg:
            # форматируем один файл
            result = f.format_file(arg, check_only=False)
            print(result.get("message", "Ошибка"))
        else:
            # форматируем весь проект
        ...
        e = Explainer()
        if arg:
            # Объясняем конкретный diff
            print("[Explainer] Анализирую изменения...")
            result = e.explain_diff(arg)
            print(result)
        else:
            # Объясняем последний diff (если сохранён)
