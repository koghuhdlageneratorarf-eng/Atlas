...
from core.runtime.engine import TaskState, Workflow, WorkflowStep, get_runtime
    ...
    def _execute_task(self, task, goal):
        ...
        def step_complete(context):
            self.runtime.update_task_state(
                task.id, TaskState.DONE, result="AutoPilot completed"
            )
            # Авто-коммит
...
if __name__ == "__main__":
    ...
    ge.add_task_to_goal(
        goal.id, "Тестовая задача", "Проверка выполнения через AutoPilot"
    )
