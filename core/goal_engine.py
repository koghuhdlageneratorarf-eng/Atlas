...
from core.runtime.engine import get_runtime, TaskState, Task
...
class GoalEngine:
    ...
    def add_task_to_goal(self, goal_id: str, task_title: str, task_description: str = "") -> Optional[str]:
    ...
    def status(self) -> str:
        ...
        for goal in self.goals.values():
            tasks_done = sum(1 for t in goal.tasks if self.runtime.get_task(t) and self.runtime.get_task(t).state == TaskState.DONE)
            total = len(goal.tasks)
            status = "✅" if goal.status == "completed" else "🔄" if goal.status == "active" else "⏸️"
            lines.append(f"{status} {goal.title} [{goal.id}] — {tasks_done}/{total} задач")
        return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    # Создаём цель
    goal = ge.create_goal("Разработка Runtime Engine", "Реализовать полноценный Runtime")
    
    # Добавляем задачи
    ge.add_task_to_goal(goal.id, "Создать базовый Runtime", "Базовый класс RuntimeEngine")
