...
class Scheduler:
    ...
    def add_task(self, name: str, interval_seconds: int, action: Callable):
        ...
        self.tasks.append(ScheduledTask(name=name, interval_seconds=interval_seconds, action=action))
        print(f"[Scheduler] Добавлена задача: {name} (каждые {interval_seconds//60} мин)")
            ...
            for task in self.tasks:
                ...
                if task.last_run is None or (now - task.last_run).total_seconds() >= task.interval_seconds:
                    try:
                        task.action()
                        task.last_run = now
                        print(f"[Scheduler] Выполнена: {task.name} в {now.strftime('%H:%M:%S')}")
